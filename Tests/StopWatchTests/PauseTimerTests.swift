...
final class PauseTimerTest: XCTestCase {}

extension PauseTimerTest {
    func testStarting() {
        // given
        var collector = [TimeInterval]()
        let timer = PauseTimer(each: 0.1) { collector.append($0) }
    ...
    func testStopping() {
        // given
        var collector = [TimeInterval]()
        let timer = PauseTimer(each: 0.1, onUpdate: { collector.append($0) })
